...
if (!function_exists('htmLawed')) {
    ...
    function htmLawed($t, $C = 1, $S = array())
    {
        ...
        if ($x == '-*') {
            ...
            if (isset($x[1])) {
                if (strpos($x, '(')) { // Temporarily replace hyphen of custom element, minus being special character
                    $x = preg_replace_callback('`\([^()]+\)`', function($m) {
        ...
        $x = array_flip((isset($x[0]) && $x[0] == '*') ? preg_replace('`^[^*]`', '-' . '\\0', explode('/', (!empty($C['safe']) ? preg_replace('`/on[^/]+`', '', $x) : $x))) : array_filter(explode(',', $x . (!empty($C['safe']) ? ',on*' : ''))));
        $C['deny_attribute'] = $x;
        ...
        if ($C['clean_ms_char']) { // Convert MS Windows CP-1252
            ...
            $x = $x + ($C['clean_ms_char'] == 1 ? array("\x82" => '&#8218;', "\x84" => '&#8222;', "\x91" => '&#8216;', "\x92" => '&#8217;', "\x93" => '&#8220;', "\x94" => '&#8221;') : array(
                    "\x82" => '\'',
                    "\x84" => '"',
                    "\x91" => '\'',
                    "\x92" => '\'',
        ...
        $t = preg_replace_callback('`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'hl_entity', str_replace('&', '&amp;', $t));
...
if (!function_exists('hl_attributeValue')) {
    /**
    ...
    function hl_attributeValue($attr, $value, $ruleAr, $ele)
    {
        ...
        $valSep = (in_array($attr, $spacedValsAttrAr) || ($attr == 'archive' && $ele == 'object')) ? ' ' : (($attr == 'srcset' || ($attr == 'archive' && $ele == 'applet')) ? ',' : '');
        $out = array();
...
if (!function_exists('hl_balance')) {
    ...
    function hl_balance($t, $act = 1, $parentEle = 'div')
    {
        ...
        $flowKidEleAr = array('a'        => 1,
                              ...
                              'th'       => 1,
                              'video'    => 1
        ); // Later context-wise dynamic move of ins & del to $inlineKidEleAr
        $inlineKidEleAr = array('abbr'       => 1,
                                'acronym'    => 1,
                                'address'    => 1,
                                'b'          => 1,
                                'bdi'        => 1,
        ...
        $invalidMomKidAr = array('a'        => array('a' => 1, 'address' => 1, 'button' => 1, 'details' => 1, 'embed' => 1, 'iframe' => 1, 'keygen' => 1, 'label' => 1, 'select' => 1, 'textarea' => 1),
                                 'address'  => array(
                                     'address' => 1,
                                     'article' => 1,
                                     'aside'   => 1,
                                 ...
                                 'audio'    => array('audio' => 1, 'video' => 1),
                                 'button'   => array('a' => 1, 'address' => 1, 'button' => 1, 'details' => 1, 'embed' => 1, 'iframe' => 1, 'keygen' => 1, 'label' => 1, 'select' => 1, 'textarea' => 1),
                                 'dfn'      => array('dfn' => 1),
                                 'fieldset' => array('fieldset' => 1),
                                 'footer'   => array('footer' => 1, 'header' => 1),
                                 'form'     => array('form' => 1),
                                 'header'   => array('footer' => 1, 'header' => 1),
                                 'label'    => array('label' => 1),
                                 'main'     => array('main' => 1),
                                 'meter'    => array('meter' => 1),
                                 ...
                                 'rt'       => array('ruby' => 1),
                                 'time'     => array('time' => 1),
                                 'video'    => array('audio' => 1, 'video' => 1)
        );
        $invalidKidEleAr = array('a'        => 1,
                                 'address'  => 1,
                                 'article'  => 1,
                                 'aside'    => 1,
                                 'audio'    => 1,
                                 ...
                                 'time'     => 1,
                                 'video'    => 1
        ); // $invalidMomKidAr values
        $invalidMomEleAr = array_keys($invalidMomKidAr);
        $validMomKidAr = array(
            'colgroup' => array('col' => 1, 'template' => 1),
            'datalist' => array('option' => 1, 'script' => 1),
            'details'  => array('summary' => 1),
            'dir'      => array('li' => 1),
            'dl'       => array('dd' => 1, 'div' => 1, 'dt' => 1),
            'hgroup'   => array('h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1),
            'menu'     => array('li' => 1, 'script' => 1, 'template' => 1),
            'ol'       => array('li' => 1, 'script' => 1, 'template' => 1),
            'optgroup' => array('option' => 1, 'script' => 1, 'template' => 1),
            'option'   => array('#pcdata' => 1),
            'picture'  => array('img' => 1, 'script' => 1, 'source' => 1, 'template' => 1),
            'rbc'      => array('rb' => 1),
            'rp'       => array('#pcdata' => 1),
            'rtc'      => array('rp' => 1, 'rt' => 1),
            ...
            'table'    => array('caption' => 1, 'col' => 1, 'colgroup' => 1, 'script' => 1, 'tbody' => 1, 'tfoot' => 1, 'thead' => 1, 'tr' => 1, 'template' => 1),
            'tbody'    => array('script' => 1, 'template' => 1, 'tr' => 1),
            'tfoot'    => array('tr' => 1),
            'textarea' => array('#pcdata' => 1),
            'thead'    => array('script' => 1, 'template' => 1, 'tr' => 1),
            'tr'       => array('script' => 1, 'td' => 1, 'template' => 1, 'th' => 1),
            'ul'       => array('li' => 1, 'script' => 1, 'template' => 1)
        ...
        $otherValidMomKidAr = array('address'    => array('p' => 1),
                                    'applet'     => array('param' => 1),
                                    'audio'      => array('source' => 1, 'track' => 1),
                                    'blockquote' => array('script' => 1),
                                    'details'    => array('summary' => 1),
                                    'fieldset'   => array('legend' => 1, '#pcdata' => 1),
                                    'figure'     => array('figcaption' => 1),
                                    'form'       => array('script' => 1),
                                    'map'        => array('area' => 1),
                                    'legend'     => array('h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1),
                                    'object'     => array('param' => 1, 'embed' => 1),
                                    'summary'    => array('h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1, 'hgroup' => 1),
        ...
        for ($i = -1, $eleCount = count($t); ++$i < $eleCount;) {
            ...
            if (isset($ele) && ($act == 1 || (isset($validKidsOfMom['#pcdata']) && ($act == 3 || $act == 5)))) {
            ...
            if (isset($content[0])) {
                if (strlen(trim($content)) && (($queueLength && isset($blockKidEleAr[$eleNow])) || (isset($blockKidEleAr[$mom]) && !$queueLength))) {
                    ...
                    foreach (preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $content, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $m) {
                        echo(substr($m, 0, 2) == "\x01\x02" ? $m : ($act > 4 ? preg_replace('`\S`', '', $m) : ''));
                    }
                } elseif ($act > 4) {
            ...
            if (!$queueLength || !isset($invalidKidEleAr[$ele]) || !array_intersect($openEleQueue, $invalidMomEleAr)) { // If no open element; mostly immediate parent-child relation should hold
...
if (!function_exists('hl_commentCdata')) {
    /**
...
if (!function_exists('hl_deprecatedElement')) {
    /**
    ...
    function hl_deprecatedElement(&$ele, &$attrStr, $act = 1)
    {
        ...
        static $fontSizeAr = array('0'  => 'xx-small',
                                   '1'  => 'xx-small',
                                   '2'  => 'small',
                                   '3'  => 'medium',
                                   '4'  => 'large',
        ...
        if ($ele == 'font') {
            ...
            while (preg_match('`(^|\s)face\s*=\s*(\'|")?([^=]+?)\\2`i', $attrStr, $m) || preg_match('`(^|\s)face\s*=(\s*)(\S+)`i', $attrStr, $m)) {
                $attrStr = str_replace($m[0], ' ', $attrStr);
...
if (!function_exists('hl_entity')) {
    ...
    function hl_entity($t)
    {
        ...
        if ($t[0] != '#') {
            return ($C['and_mark'] ? "\x06" : '&') . (isset($reservedEntAr[$t]) ? $t : (isset($commonEntNameAr[$t]) ? (!$C['named_entity'] ? '#' . ($C['hexdec_entity'] > 1 ? 'x' . dechex($commonEntNameAr[$t]) : $commonEntNameAr[$t]) : $t) : (isset($rareEntNameAr[$t]) ? (!$C['named_entity'] ? '#' . ($C['hexdec_entity'] > 1 ? 'x' . dechex($rareEntNameAr[$t]) : $rareEntNameAr[$t]) : $t) : 'amp;' . $t))) . ';';
        }
        if (($n = ctype_digit($t = substr($t, 1)) ? intval($t) : hexdec(substr($t, 1))) < 9 || ($n > 13 && $n < 32) || $n == 11 || $n == 12 || ($n > 126 && $n < 160 && $n != 133) || ($n > 55295 && ($n < 57344 || ($n > 64975 && $n < 64992) || $n == 65534 || $n == 65535 || $n > 1114111))) {
...
if (!function_exists('hl_spec')) {
    ...
    function hl_spec($t)
    {
        ...
        if (!function_exists('hl_aux1')) {
            function hl_aux1($x)
            {
                return substr(str_replace(array(";", "|", "~", " ", ",", "/", "(", ")", '`"'), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07", "\x08", '"'), $x[0]), 1, -1);
            }
        }
        ...
        for ($i = count(($t = explode(';', $t))); --$i >= 0;) {
            $ele = $t[$i];
            if (empty($ele) || ($tagPos = strpos($ele, '=')) === false || !strlen(($tagSpec = substr($ele, $tagPos + 1)))) {
                continue;
            }
...
if (!function_exists('hl_tag')) {
    ...
    function hl_tag($t)
    {
        ...
        if ((!strpos($ele, '-') && !isset($C['elements'][$ele])) // Not custom element
            || (strpos($ele, '-') && (isset($C['elements']['-' . $ele]) || (!$C['any_custom_element'] && !isset($C['elements'][$ele])) || isset($invalidCustomEleAr[$ele]) || preg_match('`[^-._0-9a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\x{2ff}' . '\x{370}-\x{37d}\x{37f}-\x{1fff}\x{200c}-\x{200d}\x{2070}-\x{218f}' . '\x{2c00}-\x{2fef}\x{3001}-\x{d7ff}\x{f900}-\x{fdcf}\x{fdf0}-\x{fffd}\x{10000}-\x{effff}]`u', $ele)))) {
        ...
        if (!empty($m[1])) {
            return (!isset($emptyEleAr[$ele]) ? (empty($C['hook_tag']) ? "</$ele>" : call_user_func($C['hook_tag'], $ele, 0)) : ($C['keep_bad'] % 2 ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : ''));
        }
        ...
        static $attrEleAr = array(
            ...
            'cite'            => array('blockquote' => 1, 'del' => 1, 'ins' => 1, 'q' => 1),
            'classid'         => array('object' => 1),
            'clear'           => array('br' => 1),
            'code'            => array('applet' => 1),
            'codebase'        => array('applet' => 1, 'object' => 1),
            ...
            'height'          => array('applet' => 1, 'canvas' => 1, 'embed' => 1, 'iframe' => 1, 'img' => 1, 'input' => 1, 'object' => 1, 'td' => 1, 'th' => 1, 'video' => 1),
            'high'            => array('meter' => 1),
            ...
            'open'            => array('details' => 1, 'dialog' => 1),
            'optimum'         => array('meter' => 1),
            'pattern'         => array('input' => 1),
            'ping'            => array('a' => 1, 'area' => 1),
            ...
            'pubdate'         => array('time' => 1),
            'radiogroup'      => array('command' => 1),
            'readonly'        => array('input' => 1, 'textarea' => 1),
            ...
            'shape'           => array('a' => 1, 'area' => 1),
            'size'            => array('font' => 1, 'hr' => 1, 'input' => 1, 'select' => 1),
            'sizes'           => array('link' => 1),
            'span'            => array('col' => 1, 'colgroup' => 1),
            'src'             => array('audio' => 1, 'embed' => 1, 'iframe' => 1, 'img' => 1, 'input' => 1, 'script' => 1, 'source' => 1, 'track' => 1, 'video' => 1),
            ...
            'value'           => array('button' => 1, 'data' => 1, 'input' => 1, 'li' => 1, 'meter' => 1, 'option' => 1, 'param' => 1, 'progress' => 1),
            'valuetype'       => array('param' => 1),
            'vspace'          => array('applet' => 1, 'embed' => 1, 'img' => 1, 'object' => 1),
            'width'           => array('applet' => 1, 'canvas' => 1, 'col' => 1, 'colgroup' => 1, 'embed' => 1, 'hr' => 1, 'iframe' => 1, 'img' => 1, 'input' => 1, 'object' => 1, 'pre' => 1, 'table' => 1, 'td' => 1, 'th' => 1, 'video' => 1),
            'wmode'           => array('embed' => 1),
        ...
        static $urlAttrAr = array(
            ...
            'srcset'      => 1,
            'usemap'      => 1
        ); // Excludes style and on*

        // .. Deprecated.

        $alterDeprecAttr = 0;
        if ($C['no_deprecated_attr']) {
            static $deprecAttrEleAr = array(
                'align'       => array(
                    'caption' => 1,
                    'div'     => 1,
                    'h1'      => 1,
                ...
                'width'       => array('hr' => 1, 'pre' => 1, 'table' => 1, 'td' => 1, 'th' => 1)
            );
            static $deprecAttrPossibleEleAr = array(
                'a'       => 1,
                'br'      => 1,
                'caption' => 1,
                'div'     => 1,
        ...
        if ($C['lc_std_val']) {
            static $lCaseStdAttrValAr = array(
                'all'            => 1,
                'auto'           => 1,
                'baseline'       => 1,
                'bottom'         => 1,
            ...
            static $lCaseStdAttrValPossibleEleAr = array(
                'a'         => 1,
                'area'      => 1,
                'bdo'       => 1,
                'button'    => 1,
            ...
            switch ($state) {
                ...
                case 2:
                    if (preg_match('`^((?:"[^"]*")|(?:\'[^\']*\')|(?:\s*[^\s"\']+))(.*)`', $attrStr, $m)) { // Value
                        ...
                        $ok = 1;
                        $state = 0;
                        $attrAr[$attr] = trim(str_replace('<', '&lt;', ($m[0] == '"' || $m[0] == '\'') ? substr($m, 1, -1) : $m));
                    }
                    break;
            }
            if (!$ok) {
        ...
        foreach ($attrAr as $attr => $v) {
            ...
            if (
                ...
                if ($attr == 'style' && !$C['style_pass']) {
                    ...
                    $v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'hl_url', $v);
        ...
        if ($alterDeprecAttr) {
            ...
            foreach ($filtAttrAr as $name => $val) {
                ...
                if ($name == 'align') {
                    ...
                    $css[] = $name . ': ' . ((isset($val[0]) && $val[0] != '*') ? $val . (ctype_digit($val) ? 'px' : '') : 'auto');
                } elseif ($name == 'hspace') {
...
if (!function_exists('hl_tidy')) {
    ...
    function hl_tidy($t, $format, $parentEle)
    {
        ...
        if (!function_exists('hl_aux2')) {
            function hl_aux2($x)
            {
                return $x[1] . str_replace(array("<", ">", "\n", "\r", "\t", ' '), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x07"), $x[3]) . $x[4];
            }
        }
        $t = preg_replace(array('`(<\w[^>]*(?<!/)>)\s+`', '`\s+`', '`(<\w[^>]*(?<!/)>) `'), array(' $1', ' ', '$1'), preg_replace_callback(array(
                                                                                                                                               '`(<(!\[CDATA\[))(.+?)(\]\]>)`sm',
                                                                                                                                               '`(<(!--))(.+?)(-->)`sm',
        ...
        $preOpenPostCloseEleAr = array('audio'      => 1,
                                       'canvas'     => 1,
                                       'caption'    => 1,
                                       'dd'         => 1,
                                       'dt'         => 1,
...
if (!function_exists('hl_url')) {
    ...
    function hl_url($url, $attr = null)
    {
        ...
        if (preg_match('`^([^:?[@!$()*,=/\'\]]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', $url, $m) && !isset($okSchemeAr[strtolower($m[1])]) // Special crafting suggests malice
        ) {
